...
pub fn printDeclarations(
    ...
    for (declarations, 0..) |*decl, i| {
        output.print("\n[{}] {s}\n", .{ i, decl.name.in(text) });
        printTermDetailedInner(decl.term.asConst(), 0, "", text);
        output.print("\n", .{});
    }
}
...
pub fn printQueries(queries: []const Query) void {
    for (queries, 0..) |*query, i| {
        output.print("\n<{}>\n", .{i});
        printTermDetailedInner(query.term.asConst(), 0, "");
        output.print("\n", .{});
    }
}
...
fn printTermDetailedInner(
    ...
    switch (term.value) {
        ...
        .group => |inner| {
            ...
            printSpanValue(term.span, false, text);
            printTermDetailedInner(inner.asConst(), depth + 1, "", text);
        },
        .abstraction => |abstr| {
            ...
            printTermDetailedInner(abstr.body.asConst(), depth + 1, "body", text);
        },
        .application => |appl| {
            printLabel(depth, prefix, "application");
            printSpanValue(term.span, false, text);
            printTermDetailedInner(appl.function.asConst(), depth + 1, "function", text);
            printTermDetailedInner(appl.argument.asConst(), depth + 1, "argument", text);
        },
    }
}
